...
def test_one(task, model, args):
    ...
    if args.embedding != 'bilstm':
        ...
        if args.bert:
            ...
            XS, XS_inputD, XS_avg, XS_loss, XS_w2v = model['ebd'](support)
            YS = support['label']

            XQ, XQ_inputD, XQ_avg, XQ_loss, XQ_w2v = model['ebd'](query)
            YQ = query['label']
        ...
        all_label = YQ
        return acc, all_sentence_ebd.cpu().detach().numpy(), all_label.cpu().detach().numpy(), query_data.cpu().detach().numpy(), x_hat.cpu().detach().numpy()
...
def test(test_data, model, args, num_episodes, verbose=True, sampled_tasks=None):
    ...
    for task in sampled_tasks:
        if args.embedding == 'bilstm':
            acc1, sentence_ebd, sentence_label, query_data, x_hat = test_one(task, model, args)
            if count < 20:
