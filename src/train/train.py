...
def train(train_data, val_data, model, args):
    ...
    optG = torch.optim.Adam(grad_param(model, ['ebd', 'clf']), lr=args.lr_g)
    ...
    for ep in range(args.train_epochs):
        ...
        print(("{}, {:s} {:2d}, {:s} {:s}{:>7.4f} ± {:>6.4f}, "
               ...
               colored("G_grad:", "blue"), np.mean(np.array(grad['ebd'])),
    ...
    if args.save:
        ...
        torch.save(model['ebd'].state_dict(), best_path + '.G')
        torch.save(model['clf'].state_dict(), best_path + '.clf')
...
def train_one(task, model, optG, args, grad):
    ...
    for _ in range(args.n):
        if args.bert:
            ...
            XS, XS_inputD, _, XS_FCloss, XS_w2v = model['ebd'](support)
            XQ, XQ_inputD, _, XQ_FCloss, XQ_w2v = model['ebd'](query)
        YS = support['label']
        YQ = query['label']
        ...
        acc, loss= model['clf'](XS, YS, XQ, YQ)
